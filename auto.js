...
const refreshServer = async function () {
  ...
  await fetch(
    "https://backend-providers-wine.vercel.app/uber/server-generate/5",
    {
      method: "POST",
    }
  ).then(() => console.log("Uber Generation Success"));
  await fetch(
    "https://backend-providers-wine.vercel.app/heetch/server-generate/5",
    {
      method: "POST",
    }
  ).then(() => console.log("Heetch Generation Success"));
  await fetch(
    "https://backend-providers-wine.vercel.app/bolt/server-generate/5",
    {
      method: "POST",
    }
