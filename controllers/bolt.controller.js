...
const deleteAll = (req, res) => {
  Bolt.deleteMany().then((data) => {
    if (data.deletedCount > 0) {
      res.json({
        result: true,
        message: `${data.deletedCount} Bolt trips deleted from database.`,
      });
    } else {
...
const putCourseTaken = (req, res) => {
  ...
  Bolt.findOne({ course_id: req.body.course_id }).then((data) => {
...
const refreshCoursesStatus = (req, res) => {
  Bolt.updateMany({ date: { $lte: moment() } }, { status: "Passed" }).then(() =>
    Bolt.find({ status: "Passed" }).then((data) =>
      res.json({ result: true, data })
    )
  );
};
